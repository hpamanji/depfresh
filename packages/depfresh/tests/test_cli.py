...
def test_main_dispatches_addon_command(monkeypatch):
    # A registered add-on subcommand (e.g. from depfresh-pro) receives the rest
    # of argv; core never imports the add-on.
    called = {}
    ...
    monkeypatch.setattr(
        cli, "_load_addon_command", lambda name: fake_handler if name == "update" else None
    )
    rc = main(["update", "https://github.com/o/n", "--token", "t"])
    assert rc == 0
    assert called["argv"] == ["https://github.com/o/n", "--token", "t"]


def test_main_hints_when_addon_missing(capsys, monkeypatch):
    monkeypatch.setattr(cli, "_load_addon_command", lambda name: None)
    rc = main(["update", "https://github.com/o/n"])
    err = capsys.readouterr().err
    assert rc == 2
    assert "depfresh-pro" in err
