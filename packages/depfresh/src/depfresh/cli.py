...
# Subcommands provided by optional add-on packages, discovered at runtime via the
# "depfresh.commands" entry-point group. Listed here only so the core CLI can
# print a helpful hint when the add-on isn't installed — core never imports them.
_ADDON_COMMANDS = {"update": "depfresh-pro"}
...
def _load_addon_command(name: str) -> Callable[[list[str]], int] | None:
    """Return an add-on subcommand handler registered under 'depfresh.commands'."""
    for ep in importlib.metadata.entry_points(group="depfresh.commands"):
        if ep.name == name:
            return ep.load()
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    # A leading non-flag token may be an add-on subcommand (e.g. `depfresh update`).
    if argv and not argv[0].startswith("-"):
        handler = _load_addon_command(argv[0])
        if handler is not None:
            return handler(argv[1:])
        if argv[0] in _ADDON_COMMANDS:
            pkg = _ADDON_COMMANDS[argv[0]]
            print(
                f"depfresh: '{argv[0]}' needs the {pkg} package (pip install {pkg})",
                file=sys.stderr,
            )
            return 2
